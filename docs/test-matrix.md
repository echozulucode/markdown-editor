...
| WYSIWYG code language popover stability | Mobile Chromium, desktop Chromium | Switch all-modes example to WYSIWYG, focus a code block, resize and scroll | No Lexical active-editor-state runtime error is emitted | Post-MVP - regression coverage added |
| Refined hybrid properties | Mobile Chromium, desktop Chromium | Visit all-modes example, edit tags, add schema-backed property, keyboard-reorder with drag handle, inspect type popover | Compact properties editor uses schema labels/types/defaults and updates Markdown through public APIs | Post-MVP - Phase 6 coverage added |
